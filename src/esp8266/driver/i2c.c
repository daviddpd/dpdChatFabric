...
i2c_writeByte(uint8 data)
{
    ...
    ack = i2c_check_ack();
	CHATFABRIC_DEBUG_FMT(_GLOBAL_DEBUG, "ack = %d", ack);
    return ack;
}

uint8 ICACHE_FLASH_ATTR
i2c_writeWord(uint16 data)
{
	uint8 ack;
//	ack = i2c_writeByte((uint8)data[0]);
//	ack = ack & i2c_writeByte((uint8)data[1]);
	return ack;
}