#ifndef __SX1509_SparkFunSX1509_H__
#define __SX1509_SparkFunSX1509_H__
/******************************************************************************
SparkFunSX1509.h
...
/*
#define _times_r(...) __FIXME_NULLFUNC()
#define calculateLEDTRegister(...) __FIXME_NULLFUNC()
#define calculateSlopeRegister(...) __FIXME_NULLFUNC()
#define configClock(...) __FIXME_NULLFUNC()
#define debounceConfig(...) __FIXME_NULLFUNC()
#define debounceEnable(...) __FIXME_NULLFUNC()
#define debounceKeypad(...) __FIXME_NULLFUNC()
#define debouncePin(...) __FIXME_NULLFUNC()
#define debounceTime(...) __FIXME_NULLFUNC()
#define delay(...) __FIXME_NULLFUNC()
#define digitalWrite(...) __FIXME_NULLFUNC()
#define init(...) __FIXME_NULLFUNC()
#define interruptSource(...) __FIXME_NULLFUNC()
#define ledDriverInit(...) __FIXME_NULLFUNC()
#define pinDir(...) __FIXME_NULLFUNC()
#define pinMode(...) __FIXME_NULLFUNC()
#define readByte(...) __FIXME_NULLFUNC()
#define readKeyData(...) __FIXME_NULLFUNC()
#define readPin(...) __FIXME_NULLFUNC()
#define readWord(...) __FIXME_NULLFUNC()
#define reset(...) __FIXME_NULLFUNC()
#define setupBlink(...) __FIXME_NULLFUNC()
#define wiringPiI2CSetup(...) __FIXME_NULLFUNC()
#define wiringPiI2CSetupInterface(...) __FIXME_NULLFUNC()
#define wiringPiI2CWrite(...) __FIXME_NULLFUNC()
#define wiringPiI2CWriteReg16(...) __FIXME_NULLFUNC()
#define wiringPiI2CWriteReg8(...) __FIXME_NULLFUNC()
#define writeByte(...) __FIXME_NULLFUNC()
#define writePin(...) __FIXME_NULLFUNC()
#define writeWord(...) __FIXME_NULLFUNC()
*/
...
#define SX1509_constrain(amt,low,high) ((amt)<(low)?(low):((amt)>(high)?(high):(amt)))
...
typedef struct  {
	...
	byte SX1509_readByte(byte registerAddress);
	unsigned int SX1509_readWord(byte registerAddress);
	void SX1509_writeByte(byte registerAddress, byte writeValue);
	void SX1509_writeWord(byte registerAddress, unsigned int writeValue);
	byte SX1509_calculateLEDTRegister(int ms); 
	byte SX1509_calculateSlopeRegister(int ms, byte onIntensity, byte offIntensity);
...
// -----------------------------------------------------------------------------
//	byte SX1509_begin(byte address, byte resetPin);
	byte SX1509_begin(const char* device, byte address, byte resetPin);
	byte SX1509_init(const char*); // Legacy -- use begin now
...
// -----------------------------------------------------------------------------
	void SX1509_reset(bool hardware);
...
// -----------------------------------------------------------------------------
	void SX1509_pinMode(byte pin, byte inOut);
	void SX1509_pinDir(byte pin, byte inOut); // Legacy - use pinMode
...
// -----------------------------------------------------------------------------
//	void SX1509_digitalWrite(byte pin, byte highLow); 
//	void SX1509_writePin(byte pin, byte highLow); // Legacy - use digitalWrite
...
// -----------------------------------------------------------------------------
//	byte SX1509_digitalRead(byte pin);
//	byte SX1509_readPin(byte pin); // Legacy - use digitalRead
...
// -----------------------------------------------------------------------------
	void SX1509_ledDriverInit(byte pin, byte freq, bool log);
...
// -----------------------------------------------------------------------------
	void SX1509_analogWrite(byte pin, byte iOn);
	void SX1509_pwm(byte pin, byte iOn); // Legacy - use analogWrite
...
// -----------------------------------------------------------------------------
	void SX1509_setupBlink(byte pin, byte tOn, byte toff, byte onIntensity, byte offIntensity, byte tRise, byte tFall, bool log);
...
// 		- ledDriverInit should be called on the pin to be blinked before this.
// -----------------------------------------------------------------------------
	void SX1509_blink(byte pin, unsigned long tOn, unsigned long tOff, byte onIntensity, byte offIntensity);
...
// -----------------------------------------------------------------------------
	void SX1509_breathe(byte pin, unsigned long tOn, unsigned long tOff, unsigned long rise, unsigned long fall, byte onInt, byte offInt, bool log);
...
// -----------------------------------------------------------------------------
	void SX1509_keypad(byte rows, byte columns, unsigned int sleepTime, byte scanTime, byte debounceTime);
...
//		bits should be set.
// -----------------------------------------------------------------------------
	unsigned int SX1509_readKeypad();
	unsigned int SX1509_readKeyData(); // Legacy: use readKeypad();
...
//		bits should be set.
// -----------------------------------------------------------------------------
	byte SX1509_getRow(unsigned int keyData);
...
//		bits should be set.
// -----------------------------------------------------------------------------
	byte SX1509_getCol(unsigned int keyData);
...
// -----------------------------------------------------------------------------
	void SX1509_debounceConfig(byte configVaule);
...
//	Note: fOSC is set with the configClock function. It defaults to 2MHz.
// -----------------------------------------------------------------------------
	void SX1509_debounceTime(byte time);
...
// -----------------------------------------------------------------------------
	void SX1509_debouncePin(byte pin);
	void SX1509_debounceEnable(byte pin); // Legacy, use debouncePin
...
// -----------------------------------------------------------------------------	
	void SX1509_debounceKeypad(byte time, byte numRows, byte numCols);
...
// -----------------------------------------------------------------------------
	void SX1509_enableInterrupt(byte pin, byte riseFall);
...
// -----------------------------------------------------------------------------
	unsigned int SX1509_interruptSource(bool clear);

	void SX1509_clearInterrupt();
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------	
	bool SX1509_checkInterrupt(int pin);
...
// -----------------------------------------------------------------------------
	void SX1509_configClock(byte oscSource, byte oscPinFunction, byte oscFreqOut, byte oscDivider); // Legacy, use clock();
...
//
// -----------------------------------------------------------------------------
	void SX1509_clock(byte oscSource, byte oscDivider, byte oscPinFunction, byte oscFreqOut);
